...
\textbf{\texttt{vtk(OpenVR/Oculus)Camera}} - This is a derived class of the \textttt{Camera} class. \texttt{vtk(OpenVR/Oculus)Camera} gets the matrices from the VR library to use for rendering and integrates them with the model, view and world matrices from the visualization. It contains a scale and a translation that are designed to map world coordinates into the HMD space.
...
\textbf{\texttt{vtkPropPicker3D}} - Finally, the derived \texttt{vtkPropPicker3D} class determines what \texttt{Actor} or \texttt{Prop} objects VTK picks.
...
To allow VTK to function at levels needed for head-tracked rendering entailed
