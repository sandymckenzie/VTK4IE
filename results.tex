...
Widgets such as \texttt{Isosurfaces}, \texttt{Contours} and \textt{Slice} provide VTK level operations that can be carried out on the dataset. To circumvent possible interaction problems when dealing with large datasets, a low-resolution mode is provided that downsamples the dataset. This lets the end user fulfill actions quickly and then revert back to the full size, when they ready to visualize the complete dataset.
